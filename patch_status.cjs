...
const targetEnd = `{!p.paga &&`;

const startIndex = code.indexOf(targetStart);
// Find the end of the Juros Congelados block
const jurosCongeladosEndStr = `Juros congelados\n                                              </span>\n                                            </label>\n                                          )}`;
const endIndex = code.indexOf(jurosCongeladosEndStr) + jurosCongeladosEndStr.length;

if (startIndex !== -1 && endIndex !== -1) {
  const originalChunk = code.substring(startIndex, endIndex);
