...
const targetStart = `                                          <label className="flex items-center gap-2 cursor-pointer mb-2">`;
...
const iStart = code.indexOf(targetStart);
// find the block closing for Data de Congelamento
// We know there's a </div> for it.
let iEnd = code.indexOf(`"Erro ao atualizar data de congelamento",\n                                                      );\n                                                    }\n                                                  }}\n                                                  className="w-full px-2 py-1 border border-blue-200 rounded text-slate-700 focus:outline-none focus:border-blue-500 bg-white"\n                                                />\n                                              </div>\n                                          )}`, iStart);

if (iEnd === -1) {
  // Try another end match
  iEnd = code.indexOf('border-blue-500 bg-white"\n                                                />\n                                              </div>\n                                          )}', iStart);
}

if (iStart !== -1 && iEnd !== -1) {
  // get exact end index including the search string
  const exactEndStr = 'border-blue-500 bg-white"\n                                                />\n                                              </div>\n                                          )}';
  const finalIndex = code.indexOf(exactEndStr, iStart) + exactEndStr.length;
  
  const chunk = code.substring(iStart, finalIndex);
  ...
  code = code.substring(0, iStart) + replacement + code.substring(finalIndex);
